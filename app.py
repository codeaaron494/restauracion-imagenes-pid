...
if uploaded_file is not None:
    ...
    if modo_proteccion != "Ninguna":
        mascara_proteccion = np.ones(roi.shape[:2], dtype=np.uint8) * 255  # 255 = Aplicar filtro
        ...
        if modo_proteccion == "Modo 1: Máscara Manual":
            ...
            mascara_proteccion[py1:py2, px1:px2] = 0  # 0 = Proteger Rostro
    ...
    if operacion != "Ninguna":
        ...
        if operacion == "Reparación de Grietas":
            # Pasamos la máscara aquí también para optimizar el algoritmo de Canny internamente
        ...
        # PASO B: Aplicar post-procesamiento de protección si está activo
    ...
    if mascara_debug is not None:
        st.subheader("Depuración: Máscara de Grietas Generada")
